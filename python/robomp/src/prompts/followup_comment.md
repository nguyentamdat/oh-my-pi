...
- **New repro info?** Re-run via `repro_record`, then `forge_post_comment` with the outcome.
...
- **PR change requested?** Amend `{{workspace.branch}}` and push only for an already-open PR / authorized implementation; NEVER open a second PR, and NEVER open the first PR for an unauthorized enhancement/proposal. Reply with a short `forge_post_comment` naming what changed.
- **Confirmation or unrelated question?** Reply with one `forge_post_comment`. Leave code untouched.
