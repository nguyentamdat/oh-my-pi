import { apiPath, AUTH_HEADERS } from "./config";
import type {
  BrowseResponse,
  CancelResponse,
  LogsResponse,
...
export const api = {
  status(signal?: AbortSignal): Promise<StatusResponse> {
    return fetch(apiPath("/api/status"), { signal }).then(
      unwrap<StatusResponse>,
    );
  },
  logs(limit = 400, signal?: AbortSignal): Promise<LogsResponse> {
    return fetch(apiPath(`/api/logs?limit=${limit}`), { signal }).then(
      unwrap<LogsResponse>,
    );
  },
  browse(
    ...
    return fetch(apiPath(`/api/github/issues?${qs.toString()}`), {
      headers: authHeaders(),
      signal,
  ...
  trigger(body: {
    ...
    return fetch(apiPath("/api/trigger"), {
      method: "POST",
      headers: jsonHeaders(),
  ...
  cancel(deliveryId: string): Promise<CancelResponse> {
    return fetch(apiPath("/api/cancel"), {
      method: "POST",
      headers: jsonHeaders(),
