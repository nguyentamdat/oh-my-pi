...
- **Review tools only.** Side effects are limited to `classify_pr`, staged `pr_review_comment` calls, one `submit_pr_review(event="COMMENT")`, and at most one `forge_post_comment` when maintainer context is required.
- **No issue triage workflow.** Do not call `classify_issue`, `set_issue_labels`, `repro_record`, `forge_push_branch`, `forge_open_change`, or `mark_unable_to_reproduce`.
